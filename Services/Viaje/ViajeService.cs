...
using static GestorViajes.Services.Roadtrip.ViajeService;
using System.Linq.Expressions;
using GestorViajes.Automapper;
...
namespace GestorViajes.Services.Roadtrip
{
    public class ViajeService
    {
        ...
        public class TripService : ITripService
        {
            ...
            public async Task<GenericResponse<List<ViajeViewModel>>> List(Expression<Func<viajes, bool>>? predicate = null)
            {
                var response = await _tripRepository.List(predicate);

                if (!response.Success)
                    return new GenericResponse<List<ViajeViewModel>> { Error = response.Error };

                var viewModels = _mapper.Map<List<ViajeViewModel>>(response.Data);

                return new GenericResponse<List<ViajeViewModel>> { Data = viewModels };
            }

            public async Task<GenericResponse<ViajeViewModel>> Get(long id)
            {
                var response = await _tripRepository.Get(v => v.id == id);

                if (!response.Success)
                    return new GenericResponse<ViajeViewModel> { Error = response.Error };

                var viewModel = _mapper.Map<ViajeViewModel>(response.Data);

                return new GenericResponse<ViajeViewModel> { Data = viewModel };
            }

            public async Task<GenericResponse<ViajeViewModel>> Add(ViajeViewModel model)
            {
                // Validar existencia del conductor
                var conductor = await _userRepository.Get(u => u.Id == model.ConductorId);
                if (!conductor.Success || conductor.Data == null)
                {
                    return new GenericResponse<ViajeViewModel>
                    {
                        Error = new ErrorResponse("El conductor especificado no existe.")
                    };
                }

                // Validar existencia del vehículo
                var vehiculo = await _vehicleRepository.Get(v => v.id == model.VehiculoId);
                if (!vehiculo.Success || vehiculo.Data == null)
                {
                    return new GenericResponse<ViajeViewModel>
                    {
                        Error = new ErrorResponse("El vehículo especificado no existe.")
                    };
                }

                // Mapear y guardar
                var entity = _mapper.Map<viajes>(model);
                var saveResponse = await _tripRepository.Add(entity);

                if (!saveResponse.Success)
                    return new GenericResponse<ViajeViewModel> { Error = saveResponse.Error };

                var viewModel = _mapper.Map<ViajeViewModel>(saveResponse.Data);
                return new GenericResponse<ViajeViewModel> { Data = viewModel };
            }
